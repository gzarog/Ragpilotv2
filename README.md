...
Phase 2 (this repository's current state) adds Tree-sitter-based code
intelligence: `ragpilot index` now parses recognized source files, extracts
classes/interfaces/structs/enums, functions/methods/properties/fields,
imports, inheritance, and calls into a normalized entity/relationship graph
(stored alongside an FTS5 index over symbol names), and exposes it read-only
via `ragpilot symbol|callers|callees|references`. Fully supported languages:
...
isolation, and never aborts the run. Document parsing (Docling) and the MCP
